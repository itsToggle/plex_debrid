...
class media:
    ...
    def download(self, retries=0, library=[], parentReleases=[]):
        ...
        self.Releases = []
        if self.type == 'movie':
            if (len(self.uncollected(library)) > 0 or self.version_missing()) and self.version_missing():
            ...
            if self.released() and (not self.collected(library) or self.version_missing()) and not self.watched() and self.version_missing():
                self.isanime()
