    ...
    class media:
        ...
        def watch(self):
            if not self in content.media.ignore_queue:
                ...
                ui.print('retrying download in 30min for item: ' + self.query() + ' - attempt ' + str(
        ...
        def collected(self, list):
            if self.watchlist == plex.watchlist and content.libraries.active == ['Plex Library']:
                ...
                try:
                    ...
                    ui.print("trakt error: (trakt to trakt library check exception): " + str(e),
        ...
        def uncollected(self, list):
            if self.type == 'movie':
                ...
                for season in Seasons[:]:
                    if not season.collected(
                            list) and not season.watched() and season.released() and not season.downloading():
                        for episode in season.Episodes[:]:
                            if episode.collected(
                                    list) or episode.watched() or not episode.released() or episode.downloading():
...
class plex(content.services):
        ...
        def __init__(self, other):
            ...
            while len(self.Episodes) < self.leafCount:
                url = 'https://metadata.provider.plex.tv/library/metadata/' + self.ratingKey + '/children?includeUserState=1&X-Plex-Container-Size=200&X-Plex-Container-Start=' + str(
                    len(self.Episodes)) + '&X-Plex-Token=' + plex.users[0][1]
                response = plex.get(url)
    ...
    class library(content.libraries):
        ...
        def __new__(self):
            ...
            if not plex.library.check == [['']] and not plex.library.check == []:
                ui.print(
                    '[plex] getting plex library section/s "' + ','.join(x[0] for x in plex.library.check) + '" ...')
                types = ['1', '2', '3', '4']
                for section in plex.library.check:
                    ...
                    section_response = []
                    for type in types:
                        url = plex.library.url + '/library/sections/' + section[
                            0] + '/all?type=' + type + '&X-Plex-Token=' + plex.users[0][1]
                        response = plex.get(url)
                        if hasattr(response, 'MediaContainer'):
                            if hasattr(response.MediaContainer, 'Metadata'):
                    ...
                    if len(section_response) == 0:
                        ui.print("[plex error]: couldnt reach local plex library section '" + section[
                            0] + "' at server address: " + plex.library.url + " - or this library really is empty.")
                    else:
                ...
                if hasattr(response, 'MediaContainer'):
                    ...
                    ui.print(
                        "[plex error]: couldnt reach local plex server at server address: " + plex.library.url + " - or this library really is empty.")
            if len(list) == 0:
                ui.print(
    ...
    def match(query, type, library=[]):
        ...
        url = plex.library.url + '/library/metadata/' + some_local_media.ratingKey + '/matches?manual=1&title=' + query + '&agent=' + agent + '&language=en-US&X-Plex-Token=' + \
              plex.users[0][1]
        response = plex.get(url)
        try:
...
class trakt(content.services):
    ...
    def logerror(response):
        ...
        if response.status_code == 401:
            ui.print("[trakt] error: (401 unauthorized): trakt api key for user '" + trakt.current_user[
                0] + "' does not seem to work. Consider re-authorizing plex_debrid for this trakt user.")

    def get(url):
        try:
            response = trakt.session.get(url, headers={
                'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.102 Safari/537.36',
                'Content-type': "application/json", "trakt-api-key": trakt.client_id, "trakt-api-version": "2",
                "Authorization": "Bearer " + trakt.current_user[1]})
    ...
    def post(url, data):
        try:
            response = trakt.session.post(url, headers={
                'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.102 Safari/537.36',
                'Content-type': "application/json", "trakt-api-key": trakt.client_id, "trakt-api-version": "2",
                "Authorization": "Bearer " + trakt.current_user[1]}, data=data)
            trakt.logerror(response)
            response = json.loads(response.content, object_hook=lambda d: SimpleNamespace(**d))
            time.sleep(1)
        except:
    ...
    def oauth(code=""):
        if code == "":
            ...
            response = None
            while response == None:
                response = trakt.post('https://api.trakt.tv/oauth/device/token', json.dumps(
                    {'code': code, 'client_id': trakt.client_id, 'client_secret': trakt.client_secret}))
                time.sleep(1)
        ...
        def __init__(self):
            ...
            for list in trakt.lists:
                ...
                if list_type == "watchlist":
                    try:
                        watchlist_items, header = trakt.get(
                        ...
                        for element in watchlist_items:
                            if hasattr(element, 'show'):
                                ...
                                if not element.show in self.data:
                                    refresh = True
                                    ui.print(
                                        '[trakt] item: "' + element.show.title + '" found in ' + trakt.current_user[
                                            0] + "'s trakt watchlist.")
                                    self.data.append(trakt.show(element.show))
                                ...
                                if not element.movie in self.data:
                                    refresh = True
                                    ui.print(
                                        '[trakt] item: "' + element.movie.title + '" found in ' + trakt.current_user[
                                            0] + "'s trakt watchlist.")
                                    self.data.append(trakt.movie(element.movie))
    ...
    class library(content.libraries):
        ...
        def add(original_element):
            ...
            data = {'movies': movies, 'shows': shows}
            response = trakt.post('https://api.trakt.tv/sync/collection',
...
class overseerr(content.services):
    ...
    def setup(self):
        ...
        if len(plex.users) == 0:
            ...
            return
        try:
            response = overseerr.session.get(overseerr.base_url + '/api/v1/request',
                                             headers={"X-Api-Key": overseerr.api_key}, timeout=0.5)
        ...
        while not working_url:
            if overseerr.base_url == "http://localhost:5055":
                print(
                    "Looks like overseerr couldn't be reached under the default base url ('" + overseerr.base_url + "').")
            else:
                print(
                    "Looks like overseerr couldn't be reached under the current base url ('" + overseerr.base_url + "').")
            print("Please make sure overseerr is running and try again, or provide your overseerr base URL below:")
            print()
            overseerr.base_url = input(
    ...
    def get(url):
        try:
            response = overseerr.session.get(url, headers={
                'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.102 Safari/537.36',
                'Content-type': "application/json", "X-Api-Key": overseerr.api_key})
    ...
    def post(url, data):
        try:
            response = overseerr.session.post(url, headers={
                'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.102 Safari/537.36',
                'Content-type': "application/json", "X-Api-Key": overseerr.api_key}, data=data)
            overseerr.logerror(response)
            response = json.loads(response.content, object_hook=lambda d: SimpleNamespace(**d))
        except:
            response = None
                ...
                try:
                    ...
                    for element in response.results:
                        if not element in self.data and (
                                element.requestedBy.displayName in overseerr.users or overseerr.users == ['all']) and [
                            str(element.status)] in overseerr.allowed_status:
...
class debrid:
    ...
    def download(element: content.media, stream=True, query='', force=False):
        ...
        if stream:
            ...
            for index, version in enumerate(versions):
                ...
                if not version == ['(.*)']:
                    ui.print(
                        'attempting to download version ' + str(index + 1) + '/' + str(len(versions)) + ': "' + version[
                            0] + '" ...')
                for release in cached_releases:
                    if version[0].startswith('!'):
                        try:
                            ...
                            ui.print('multiple versions error: check your multiple versions settings.',
                                     debug=ui_settings.debug)
                            match = True
                    else:
                        try:
    ...
    class realdebrid(services):
        ...
        def get(url):
            headers = {
                'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.102 Safari/537.36',
                'authorization': 'Bearer ' + debrid.realdebrid.api_key}
            try:
            ...
            for release in cached[:]:
                ...
                if regex.match(r'(' + query.replace('.', '\.').replace("\.*", ".*") + ')', release.title,
                    ...
                    if stream:
                        ...
                        for version in release.files:
                            if hasattr(version, 'files'):
                                if len(version.files) > 0 and version.wanted > len(wanted) / 2 or force:
                                    ...
                                    try:
                                        response = debrid.realdebrid.post(
                                    ...
                                    response = debrid.realdebrid.get(
                        ...
                        try:
                            response = debrid.realdebrid.post('https://api.real-debrid.com/rest/1.0/torrents/addMagnet',
                                                              {'magnet': release.download[0]})
                            time.sleep(0.1)
                            debrid.realdebrid.post(
                            ...
                            if len(getattr(response, release.hash.lower()).rd) > 0:
                                for cashed_version in getattr(response, release.hash.lower()).rd:
                                    ...
                                    for file in cashed_version.__dict__:
                                        debrid_file = debrid.realdebrid.file(file,
                                                                             getattr(cashed_version, file).filename,
                                                                             getattr(cashed_version, file).filesize,
                                                                             wanted, unwanted)
    ...
    class alldebrid(services):
        ...
        def get(url):
            headers = {
                'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.102 Safari/537.36',
                'authorization': 'Bearer ' + debrid.alldebrid.api_key}
            try:
    ...
    class premiumize(services):
        ...
        def post(url, data):
            ...
            try:
                response = debrid.premiumize.session.post(url + '?apikey=' + debrid.premiumize.api_key + data,
        ...
        def check(element, force=False):
            ...
            for release in element.Releases[:]:
                if len(release.hash) == 40:
                    ...
                    element.Releases.remove(release)
            if len(hashes) > 0:
                response = debrid.premiumize.get(
    ...
    class debridlink(services):
        ...
        def get(url):
            headers = {
                ...
                'Authorization': 'Bearer ' + debrid.debridlink.api_key}
            try:
        ...
        def oauth(code=""):
            if code == "":
                response = debrid.debridlink.post('https://debrid-link.fr/api/oauth/device/code',
                                                  'client_id=' + debrid.debridlink.client_id)
                return response.device_code, response.user_code
            else:
                response = None
                while response == None:
                    response = debrid.debridlink.post('https://debrid-link.fr/api/oauth/token',
                                                      'client_id=' + debrid.debridlink.client_id + '&code=' + code + '&grant_type=http%3A%2F%2Foauth.net%2Fgrant_type%2Fdevice%2F1.0')
        ...
        def check(element, force=False):
            ...
            for release in element.Releases[:]:
                if len(release.hash) == 40:
                    ...
                    element.Releases.remove(release)
            if len(hashes) > 0:
                response = debrid.debridlink.get(
    ...
    class putio(services):
        ...
        def get(url):
            headers = {
                'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.102 Safari/537.36',
                'Content-Type': 'application/x-www-form-urlencoded', 'Authorization': 'Bearer ' + debrid.putio.api_key}
            try:
        ...
        def __new__(self, scraped_releases: list):
            try:
                ...
                for group, attribute, type, descending in reversed(releases.sort.ranking):
                    scraped_releases.sort(key=lambda s: s.rank[index][attribute + ': ' + group],
                                          reverse=int(descending))
                    index += -1
                size_max = 2000
                size_min = 0
                try:
                ...
                ui.print(
                    "sorting error: (sorting exception): something seems wrong with your sorting rules. Aborted sorting. For details enable debug printing.")
                ...
                while not hasattr(response, "torrent_results") and retries < 4:
                    if regex.search(r'(tt[0-9]+)', query, regex.I):
                        url = 'https://torrentapi.org/pubapi_v2.php?mode=search&search_imdb=' + str(
                            query) + '&ranked=0&category=52;51;50;49;48;45;44;41;17;14&token=' + scraper.rarbg.token + '&limit=100&format=json_extended&app_id=fuckshit'
                    else:
                ...
                if hasattr(response, "torrent_results"):
                    for result in response.torrent_results:
                        if regex.match(r'(' + altquery.replace('.', '\.').replace("\.*", ".*") + ')', result.title,
                                       regex.I):
                            release = releases('[rarbg]', 'torrent', result.title, [], float(result.size) / 1000000000,
                        ...
                        for count, torrent in enumerate(torrentList):
                            ...
                            if regex.match(r'(' + altquery.replace('.', '\.').replace("\.*", ".*") + ')', title,
                                ...
                                if regex.search(r'([0-9]*?\.[0-9])(?= MB)', size, regex.I):
                                    ...
                                    size = float(size)
                                scraped_releases += [
                                    releases('[1337x]', 'torrent', title, [], size, [download], seeders=int(seeders))]
                except:
                    response = None
                        ...
                        if regex.match(r'(' + altquery.replace('.', '\.').replace("\.*", ".*") + ')', result.Title,
                            ...
                            if not result.MagnetUri == None:
                                if not result.Tracker == None and not result.Size == None:
                                    scraped_releases += [
                                        releases('[jackett: ' + str(result.Tracker) + ']', 'torrent', result.Title, [],
                                    ...
                                    scraped_releases += [releases('[jackett: unnamed]', 'torrent', result.Title, [],
                                                                  float(result.Size) / 1000000000, [result.MagnetUri],
                                                                  seeders=result.Seeders)]
                ...
                if 'Location' in link.headers:
                    if regex.search(r'(?<=btih:).*?(?=&)', str(link.headers['Location']), regex.I):
                        if not result.Tracker == None and not result.Size == None:
                            ...
                            scraped_releases += [
                                releases('[jackett: ' + str(result.Tracker) + ']', 'torrent', result.Title, [], 1,
                    ...
                    if not result.Tracker == None and not result.Size == None:
                        ...
                        scraped_releases += [
                            releases('[jackett: ' + str(result.Tracker) + ']', 'torrent', result.Title, [], 1, [magnet],
                                     seeders=result.Seeders)]
                    elif not result.Size == None:
                        scraped_releases += [
                            releases('[jackett: unnamed]', 'torrent', result.Title, [], float(result.Size) / 1000000000,
                            ...
                            if hasattr(result, 'magnetUrl'):
                                if not result.magnetUrl == None:
                                    if not result.indexer == None and not result.size == None:
                                        scraped_releases += [
                                            releases('[prowlarr: ' + str(result.indexer) + ']', 'torrent', result.title,
                                        ...
                                        scraped_releases += [
                                            releases('[prowlarr: unnamed]', 'torrent', result.title, [],
                                                     float(result.size) / 1000000000, [result.magnetUrl],
                                                     seeders=result.seeders)]
                ...
                if 'Location' in link.headers:
                    if regex.search(r'(?<=btih:).*?(?=&)', str(link.headers['Location']), regex.I):
                        if not result.indexer == None and not result.size == None:
                            ...
                            scraped_releases += [
                                releases('[prowlarr: ' + str(result.indexer) + ']', 'torrent', result.title, [], 1,
                                         [link.headers['Location']], seeders=result.seeders)]
                        elif not result.size == None:
                            scraped_releases += [releases('[prowlarr: unnamed]', 'torrent', result.title, [],
                                                          float(result.size) / 1000000000, [link.headers['Location']],
                                                          seeders=result.seeders)]
                    return scraped_releases
                elif link.headers['Content-Type'] == "application/x-bittorrent":
...
class ui:
    ...
    class setting:
        def __init__(self, name, prompt, cls, key, required=False, entry="", test=None, help="", hidden=False,
                     subclass=False, oauth=False, moveable=True, preflight=False, radio=False):
            self.name = name
            self.prompt = prompt
            self.cls = cls
            self.key = key
    ...
    settings_list = [
        ['Content Services', [
            ...
            setting('Plex users', ['Please provide a name for this Plex user: ',
                                   'Please provide the Plex token for this Plex user: '], plex, 'users', entry="user",
            ...
            setting('Plex auto remove',
                    ...
                    plex.watchlist, 'autoremove', hidden=True,
            ...
            setting('Trakt users', ['Please provide a name for this Trakt user: ',
                    ...
                    trakt, 'users', entry="user", oauth=True, hidden=True),
            setting('Trakt lists', [''], trakt, 'lists', hidden=True),
            setting('Trakt auto remove',
                    'Please choose which media type/s should be removed from your watchlist after successful download ("movie","show","both" or "none"): ',
                    trakt.watchlist, 'autoremove', hidden=True,
                    help='By default, movies are removed from your watchlist after a successful download. In this setting you can choose to automatically remove shows, shows and movies or nothing.'),
            setting('Overseerr users', ['Please choose a user: '], overseerr, 'users', entry="user",
            ...
            setting('Overseerr Base URL', 'Please specify your Overseerr base URL: ', overseerr, 'base_url',
                    hidden=True),
        ]
         ],
        ['Library Service', [
            setting('Library Service', [''], content.libraries, 'active', entry="library service", subclass=True,
            ...
            setting('Plex "movies" library',
                    ...
                    plex.library, 'movies', hidden=True),
            setting('Plex "shows" library',
                    ...
                    plex.library, 'shows', hidden=True),
            setting('Plex library check', [
                'Please specify a library section number that should be checked for existing content before download: '],
                    plex.library, 'check', hidden=True, entry="section",
        ...
        ['Scraper Settings', [
            ...
            setting('Multiple versions', ['Please specify a version by entering a regex match group: '], releases.sort,
                    'multiple_versions', entry='version',
                    help='This setting allows plex_debrid to download more than one version of your requested content. For each version you specify, plex_debrid will try to download the best release that matches your version definition. You can negate version rules by adding a "!" as the first character. (Example: download an HDR and a non-HDR version: add "(\.HDR\.)" and "!(\.HDR\.)" as your rules.)'),
            setting('Maximum release size (Gb)',
                    'Please enter a maximum release size in Gb (e.g. enter 0.1 for 100Mb): ', releases.sort,
                    'size_max'),
            setting('Minimum release size (Gb)',
                    'Please enter a minimum release size in Gb (e.g. enter 0.1 for 100Mb): ', releases.sort,
                    'size_min'),
            setting('Special character renaming', ['Please specify a character or string that should be replaced: ',
                                                   'Please specify with what character or string it should be replaced: '],
                    releases.rename, 'replaceChars', entry="rule",
                    help='In this setting you can specify a character or a string that should be replaced by nothing, some other character or a string.'),
            setting('Rarbg API Key', 'The Rarbg API Key gets refreshed automatically, enter the default value: ',
                    scraper.rarbg, 'token', hidden=True),
            setting('Jackett Base URL', 'Please specify your Jackett base URL: ', scraper.jackett, 'base_url',
                    hidden=True),
            setting('Jackett API Key', 'Please specify your Jackett API Key: ', scraper.jackett, 'api_key',
                    hidden=True),
            setting('Jackett Indexers', ['Please specify a Jackett indexer (Leave blank to scrape all indexers): '],
                    scraper.jackett, 'indexers', hidden=True, entry="indexer"),
            setting('Jackett Categories',
                    ['Please specify a Jackett torrent category (Leave blank to scrape all categories): '],
                    scraper.jackett, 'categories', hidden=True, entry="category"),
            setting('Jackett Filters', ['Please specify a Jackett indexer filter (Leave blank to not apply filter): '],
                    scraper.jackett, 'categories', hidden=True, entry="filter"),
            setting('Prowlarr Base URL', 'Please specify your Prowlarr base URL: ', scraper.prowlarr, 'base_url',
                    hidden=True),
            setting('Prowlarr API Key', 'Please specify your Prowlarr API Key: ', scraper.prowlarr, 'api_key',
                    hidden=True),
        ]
         ],
        ['Debrid Services', [
            ...
            setting('Uncached Release Download', 'Please enter "true" or "false": ', debrid, 'uncached',
                    help='Please specify wether uncached releases should be added to your debrid service, if no cached releases were found.'),
            setting(
            ...
            setting('Real Debrid API Key', 'Please enter your Real Debrid API Key: ', debrid.realdebrid, 'api_key',
                    hidden=True),
            setting('All Debrid API Key', 'Please enter your All Debrid API Key: ', debrid.alldebrid, 'api_key',
                    hidden=True),
            setting('Premiumize API Key', 'Please enter your Premiumize API Key: ', debrid.premiumize, 'api_key',
                    hidden=True),
            setting('Debrid Link API Key',
                    ...
                    debrid.debridlink, 'api_key', hidden=True, oauth=True),
            setting('Put.io API Key',
                    ...
                    debrid.putio, 'api_key', hidden=True, oauth=True),
        ]
         ],
        ['UI Settings', [
