...
try:
    ...
    print(
        "You need to install 'pip' (https://pip.pypa.io/en/stable/installation/) and run the command 'pip install " + e.name + "'.")
    ...
    class media:
        ...
        def uncollected(self, list):
            if self.type == 'movie':
                ...
                if self.collected(list) and not (
                        len(self.versions()) > 0 and not len(self.versions()) == len(releases.sort.versions)):
                    return []
                Seasons = copy.deepcopy(self.Seasons)
                for season in Seasons[:]:
                    if (not season.collected(list) or (len(season.versions()) > 0 and not len(season.versions()) == len(
                            releases.sort.versions))) and not season.watched() and season.released() and not season.downloading():
                        for episode in season.Episodes[:]:
                            if (episode.collected(list) and not (
                                    len(episode.versions()) > 0 and not len(episode.versions()) == len(
                                    releases.sort.versions))) or episode.watched() or not episode.released() or episode.downloading():
        ...
        def download(self, retries=0, library=[], parentReleases=[]):
            ...
            if self.type == 'movie':
                ...
                if self.released() and (not self.collected(library) or (
                        len(self.versions()) > 0 and not len(self.versions()) == len(
                        releases.sort.versions))) and not self.watched():
    ...
    class sort:
                        ...
                        if choice2 == '0':
                            ...
                            if not new:
                                ...
                                if choice3 in indices:
                                    ...
                                    releases.sort.version.rule.__subclasses__()[int(choice3) - 1].name
                                choice2 = '2'
                        if choice2 == '2':
                            print(
                            ...
                            if choice3 in indices:
                                default[int(choice) - 1][int(choice2) - 1] = releases.sort.version.rule.weights[
                                    int(choice3) - 1]
                            if new:
                                choice2 = '3'
                        if choice2 == '3':
        ...
        def __new__(self, scraped_releases: list, version: version):
            if len(scraped_releases) > 0:
                ...
                ui.print('sorting releases for version [' + version.name + '] ... done - found ' + str(
